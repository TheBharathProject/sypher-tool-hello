...
// With basePath: '/hello', this renders at sypher.in/hello.
// All Link hrefs are relative to the basePath — Next.js prepends /hello automatically.
// Use plain <a href="/"> to escape back to the shell apex.
    ...
    <main className="min-h-screen bg-[#0c0a09] text-[#f5f0e6] flex flex-col">
      <header className="border-b border-[rgba(245,240,230,0.1)] px-6 py-4 md:px-10">
        <div className="mx-auto flex max-w-[900px] items-center justify-between">
          ...
          <nav className="font-mono text-[11px] uppercase tracking-[0.18em] text-[#8a847b]">
            <a href="/" className="hover:text-[#f5f0e6] transition-colors">
              ← back to sypher.in
            </a>
          </nav>
        </div>
      ...
      <section className="flex-1 px-6 py-16 md:px-10 md:py-24">
        <div className="mx-auto max-w-[900px]">
          ...
          <h1
            className="mt-8 text-[clamp(40px,7vw,72px)] leading-[0.95]"
...
basePath: '/hello'   ← this is the unlock for asset URLs

# sypher-shell/next.config.ts
{ source: '/hello',          destination: 'https://sypher-tool-hello.vercel.app/hello' }
{ source: '/hello/:path+',   destination: 'https://sypher-tool-hello.vercel.app/hello/:path+' }

# served at:
https://sypher.in/hello       ← marketing / demo
https://sypher.in/hello/info  ← deeper path, same rewrite
https://sypher.in/hello/_next/...  ← assets, also rewritten
`}
          </pre>
        </div>
      </section>
...
function CardExternal({
  ...
  // Plain <a> to bypass basePath prepending — link to the actual sypher.in apex.
  return (
    <a
      href={href}
      className="block rounded border border-[rgba(245,240,230,0.1)] bg-[#070605] p-5 transition-colors hover:border-[#f5f0e6]"
